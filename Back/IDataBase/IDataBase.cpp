...
const int IDataBase::SendRequestToDB(std::string sQuerry) {
	...
	std::free(pSqliteStmt);
	sqlite3_open("NotesKeeper.db", &pSqliteDatabase);
	int iReturnCode = sqlite3_prepare(pSqliteDatabase, sQuerry.c_str(), -1, &pSqliteStmt, 0);
							 ...
							 const std::string &sPassword, int& iUserId)
{
	try {
		...
		std::string sQuerry = "INSERT INTO user value (\'" + sLogin + "\', \'" + sPassword + "\')";
...
const int IDataBase::AddFriend(const int iUserId, const int iFriendId)
{
	try {
		...
		if (iId != iUserId)
			return NO_SUCH_USER;
		std::string sQuerry = "SELECT userId from user where userId = " + std::to_string(iFriendId);
		SendRequestToDB(sQuerry);
		sqlite3_step(pSqliteStmt);
...
const int IDataBase::AddFriendRequest(const int iUserId, const int iFrReqId)
{
	try {
		...
		if (iId != iUserId)
			return NO_SUCH_USER;
		std::string sQuerry = "SELECT userId from user where userId = " + std::to_string(iFrReqId);
		SendRequestToDB(sQuerry);
		sqlite3_step(pSqliteStmt);
		iId = sqlite3_column_int(pSqliteStmt, 0);
		if (iId != iFrReqId)
					    	 ...
					    	 std::vector<std::string> &vsNote)
{
	try {
		...
		if (iId != iNoteId)
			return NO_SUCH_USER;
		std::string sQuerry = "SELECT userId from category where categoryId = ";
